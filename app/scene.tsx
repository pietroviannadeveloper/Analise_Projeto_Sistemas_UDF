...
type Props={night:boolean;playing:boolean;tour:boolean;route:boolean;colorblind:boolean;spaces:Space[];selected:string;onTraffic:(id:string,occupied:boolean|null,message:string)=>void;onSpace:(s:Space)=>void};
...
const person=(shirt:number)=>{const group=new THREE.Group(),body=mat(shirt,{roughness:.85}),skin=mat(0xe3b48c,{roughness:.75});const tg=new THREE.CapsuleGeometry(.22,.55,4,8);geos.push(tg);const torso=new THREE.Mesh(tg,body);torso.position.y=.85;torso.castShadow=true;group.add(torso);const hg=new THREE.SphereGeometry(.16,10,8);geos.push(hg);const head=new THREE.Mesh(hg,skin);head.position.y=1.42;head.castShadow=true;group.add(head);scene.add(group);return group;};
const walkers=[0x3a6ea5,0x8a4b3d,0x4e6b4e,0xb08a2e,0x5b4b7a,0x2e7a7a].map((c,i)=>{const x=[-2.2,-1.1,-.3,.3,1.1,2.2][i],g=person(c);g.position.set(x,.5,-1);return{g,x,speed:.00016+i*.00003,phase:i*1.9};});
...
if(p.playing&&!reduced)walkers.forEach(w=>{const a=time*w.speed+w.phase,u=(Math.sin(a)+1)/2;w.g.position.set(w.x,.5+Math.abs(Math.sin(a*7))*.06,THREE.MathUtils.lerp(-17,15,u));w.g.rotation.y=Math.cos(a)>=0?0:Math.PI;});
